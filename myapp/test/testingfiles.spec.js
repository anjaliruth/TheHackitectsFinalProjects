...
test("should allow the user to scroll down the page onto the dog grid", async ({
  page,
}) => {
  await page.goto("http://localhost:3000/");
...
test("find the more info button on the dog card and click on it", async ({
  page,
}) => {
  await page.goto("http://localhost:3000/");
...
test("should allow the user to click on the more info button and see the dog details", async ({
  page,
}) => {
  await page.goto("http://localhost:3000/");
  ...
  const h2Id = currentURL.split("/").pop(); // Extract the ID from the URL
  expect(idFromURL).toBe(h2Id); // Compare the ID from the URL with the received ID
